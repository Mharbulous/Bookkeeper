...
const ERROR_MESSAGES = {
  [ERROR_TYPES.WORKER_ERROR]: {
    ...
    description:
      'The background processing engine encountered an error and could not complete the operation.',
  ...
  [ERROR_TYPES.TIMEOUT]: {
    ...
    description:
      'The file processing operation took too long and was cancelled to prevent browser freezing.',
  ...
  [ERROR_TYPES.BROWSER_ERROR]: {
    ...
    description:
      'Your browser does not support some features required for optimal file processing.',
...
function determineErrorType(errorMessage, context) {
  ...
  // Timeout errors
  if (lowerMessage.includes('timeout') || lowerMessage.includes('timed out') || context.timeout) {
